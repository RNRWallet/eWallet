...
eHailing's microservices architecture requires a robust, scalable, and feature-rich database solution that can support diverse data patterns across multiple services. The system has specific requirements:

- **Complex Data Relationships**: Riders, Drivers, Rides, Vehicles, ... and user relationships
...
- **Development Experience**: Excellent Kotlin/Java integration and tooling support
...
Adopt **PostgreSQL** as the primary relational database for all services requiring persistent data storage, with specialized data stores (Redis) for specific use cases.
...
| **Booking**      | Event sourcing and order aggregates   | ACID transactions, event storage              |
...
1. **Kotlin/Java Integration**: Excellent support through Hibernate and Panache
...
graph TB
    subgraph "Service Databases"
        ...
        BookingDB[(Booking DB<br/>Event Store<br/>Aggregates)]
...
- **Migration Strategy**: Code-first migrations with version control using liquibase
- **Query Optimization**: SQL with performance monitoring
...
- [ADR-013: Event Sourcing for Wallet Service](./adr-013-wallet-eventsoucring)
