...
|----------------|------------------------------------|-------------------------------------------------------------------|-------------------------------------------------------------|------------|---------------------------------------------------------------------------|
| **Quarkus**    | Cloud-native application framework | Simplified development, built-in observability, service discovery | Foundation for service orchestration, telemetry integration | Low        | Ecosystem still evolving, fewer samples/community resources               |
...
| **Kafka**      | Message broker                     | Reliable message delivery, mature ecosystem, clustering support   | Enables event-driven architecture                           | Medium     | Operational overhead, monitoring complexity                               |
...
| **Vertical Slice Architecture** | Required code organization pattern         | Promotes modularity, enables CQRS and feature-centric development | Simplifies testing, clearer ownership of features | No         | May feel less familiar to developers used to layered architecture |
...
| **Code Quality Standards**      | Static analysis, style guidelines, linting | Improves maintainability and consistency across teams             | Shared practices across contributors/teams        | Medium     | May slow down initial development if tooling is strict            |
...
| **Secrets Management**   | Use Hashicorp Key Vault  ??         | Secure secret storage, rotation policies    | Reduces risk of leaks, supports auditing | No         | Extra cost, requires managed service access                |
...
| **Centralized Dashboards** | Grafana dashboards                | Proactive monitoring and visibility         | Improves reliability and reduces MTTR       | Medium     | Higher storage costs, risk of noise if not curated carefully   |
...
<p style={{ textAlign: "justify" }}>
  These architecture constraints establish the <b>non-negotiable foundation</b> of the system while making clear which areas are <b>flexible</b>. They balance innovation (e.g., Quarkus, Kotlin) with proven practices (e.g., PostgreSQL, DDD), and provide guardrails for teams to build consistently and securely within agreed boundaries.
</p>
